...
<div class="space-y-6">
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            ...
                            <dd class="text-lg font-medium text-gray-900">{{ $teacherCount }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
    ...
    <div class="bg-white shadow rounded-lg overflow-hidden">
        <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
            ...
            <p class="mt-1 text-sm text-gray-500">Tous les utilisateurs du système</p>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="table-row-hover">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="shrink-0 h-10 w-10 bg-blue-500 rounded-full flex items-center justify-center">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    ...
                                    {{ $user->role->name == 'director' ? 'bg-blue-100 text-blue-800' : '' }}
                                    {{ $user->role->name == 'head_teacher' ? 'bg-green-100 text-green-800' : '' }}
                                    {{ $user->role->name == 'teacher' ? 'bg-yellow-100 text-yellow-800' : '' }}
                                    {{ $user->role->name == 'secretary' ? 'bg-gray-100 text-gray-800' : '' }}">
                                    {{ $user->role->name }}
                                </span>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end space-x-2">
                                @can('update', $user)
                                <a href="{{ route('users.edit', $user) }}" class="text-green-600 hover:text-green-900 transition-colors" title="Modifier">
                                ...
                                <form action="{{ route('users.destroy', $user) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" onclick="return confirmAction('Êtes-vous sûr de vouloir supprimer cet utilisateur ?')" class="text-red-600 hover:text-red-900 transition-colors" title="Supprimer">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
