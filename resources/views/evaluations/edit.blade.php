...
<div class="max-w-4xl mx-auto">
    <div class="bg-white shadow rounded-lg">
        <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
            <h3 class="text-lg font-medium leading-6 text-gray-900">Modifier l'Évaluation</h3>
            <p class="mt-1 text-sm text-gray-500">Mettez à jour les informations de l'évaluation</p>
        </div>

        <form action="{{ route('admin.evaluations.update', $evaluation) }}" method="POST" class="px-4 py-5 sm:p-6">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                    <div>
                        ...
                        <input type="date" name="exam_date" id="exam_date"
                               value="{{ old('exam_date',
                                   $evaluation->exam_date ? \Carbon\Carbon::parse($evaluation->exam_date)->format('Y-m-d') :
            ...
            <div class="mt-6 flex justify-end space-x-3">
                <a href="{{ route('admin.evaluations.show', $evaluation) }}" class="btn-secondary">
                    Annuler
                </a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateMarks() {
        ...
        if (passMarks > maxMarks) {
            ...
            passMarksInput.classList.add('border-red-500');
        } else {
            passMarksInput.setCustomValidity('');
            passMarksInput.classList.remove('border-red-500');
        }
    }
    ...
    // Auto-select current school year if not set
